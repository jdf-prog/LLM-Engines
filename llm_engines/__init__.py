...
def get_call_worker_func(
    ...
    """
    if engine == "openai":
        from .openai_text import call_worker_openai
        ...
        from .together import call_worker_together
        call_model_worker = call_worker_together
    elif engine in ["vllm", "sglang"]:
        if engine == "vllm":
            from .vllm import launch_vllm_worker, call_vllm_worker
            call_worker_func = call_vllm_worker
            launch_worker_func = launch_vllm_worker
        elif engine == "sglang":
            from .sglang import launch_sglang_worker, call_sglang_worker
            call_worker_func = call_sglang_worker
        ...
        if worker_addr is None:
            ...
            print(f"Using existing worker at {worker_addr}")
